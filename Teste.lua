...
-- Função global para verificação externa
getgenv()._g = function()
    return {
        key = "" -- Deixe vazio ou preencha com a key válida externamente
    }
end

-- Verificação direta se a key externa está correta
local link_key = "key(isto tudo e uma key so copia e põe lá)"
if _g().key == link_key then
    loadstring(game:HttpGet("https://raw.githubusercontent.com/Lucasggk/hub/main/Main.hub.loader.lua", true))()
    return
end

-- Sistema normal (caso a key externa não esteja definida ou esteja incorreta)
